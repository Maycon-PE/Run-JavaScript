/* DATA */
import { carsList, pilotsList } from './source/data/index.js'
...
/* CONTROLLERS */
import {
  escCarro1, escCarro2, escCarro3, escCarro4,
  escPiloto1, escPiloto2, escPiloto3, escPiloto4
} from './source/controllers/selects.js'

/* VIEWS */
import leftSideRender from './source/views/left-side.js'
import mainView from './source/views/main-view.js'
import { carsDetails, pilotsDetails } from './source/views/info-base.js'
import welcomeTable from './source/views/welcome-table.js'
import { selectCar, selectPiloto } from './source/views/selects.js'

/* CSS */
import informations from './source/css/show-informations.js'
import scrollEffect from './source/css/scroll-effect.js'
import fastInfo from './source/css/fast-info.js'

const musics = ['musica-fundo', 'fundo-nfsu1']

informations()

carsDetails(carsList)
pilotsDetails(pilotsList)

welcomeTable()
.then(scrollEffect)

leftSideRender({ cars: carsList, pilots: pilotsList })
  .then(html => document.querySelector('.area-info').innerHTML = html)
  .then(() => {
    const lis = document.querySelectorAll('[exibir-popup]')

    lis.forEach((li: HTMLElement) => {
      const popup = li.childNodes[1]
...
mainView()
  ...
  .then(() => {
    const musica_inicial = musics[Math.floor(Math.random() * 2)]
    document.getElementById('musica').innerHTML = `<audio autoplay loop><source src='som/${ musica_inicial }.mp3' type='audio/ogg'></audio>`
  })
  .then(() => {
    selectCar()
    selectPiloto()
    ...
    function instanciar(random = true) {
      start()
      if (!random) {
        const carro1 = new Car(carsList[funcoesDeDefinicaoCarros[0]()])
        const carro2 = new Car(carsList[funcoesDeDefinicaoCarros[1]()])
        const carro3 = new Car(carsList[funcoesDeDefinicaoCarros[2]()])
        const carro4 = new Car(carsList[funcoesDeDefinicaoCarros[3]()])
        const piloto1 = new Pilot(pilotsList[funcoesDeDefinicaoPilotos[0]()])
        const piloto2 = new Pilot(pilotsList[funcoesDeDefinicaoPilotos[1]()])
        const piloto3 = new Pilot(pilotsList[funcoesDeDefinicaoPilotos[2]()])
        const piloto4 = new Pilot(pilotsList[funcoesDeDefinicaoPilotos[3]()])
        const limit: number = document.querySelector('[limite]').value

        Corrida(carro1, carro2, carro3, carro4, piloto1, piloto2, piloto3, piloto4, limit <= 0? 2: limit)
      } else {
        const carro1 = new Car(carsList[parseInt(String((carsList.length - 1) - Math.random() * (carsList.length - 1)))])
        const carro2 = new Car(carsList[parseInt(String((carsList.length - 1) - Math.random() * (carsList.length - 1)))])
        const carro3 = new Car(carsList[parseInt(String((carsList.length - 1) - Math.random() * (carsList.length - 1)))])
        const carro4 = new Car(carsList[parseInt(String((carsList.length - 1) - Math.random() * (carsList.length - 1)))])
        const piloto1 = new Pilot({})
        const piloto2 = new Pilot({})
        const piloto3 = new Pilot({})
        const piloto4 = new Pilot({})
        const limit = parseInt(String(Math.random() * (10 - 2) + 2))
    ...
    function Corrida(carro1Param, carro2Param, carro3Param, carro4Param, piloto1Param, piloto2Param, piloto3Param, piloto4Param, distancia) {
        ...
        while (piloto1.getNome === piloto2.getNome || piloto1.getNome === piloto3.getNome || piloto1.getNome === piloto4.getNome || piloto2.getNome === piloto3.getNome || piloto2.getNome === piloto4.getNome || piloto3.getNome === piloto4.getNome) {
          piloto1 = new Pilot(pilotsList[parseInt(String((pilotsList.length - 1) - Math.random() * (pilotsList.length - 1)))])
          piloto2 = new Pilot(pilotsList[parseInt(String((pilotsList.length - 1) - Math.random() * (pilotsList.length - 1)))])
          piloto3 = new Pilot(pilotsList[parseInt(String((pilotsList.length - 1) - Math.random() * (pilotsList.length - 1)))])
          piloto4 = new Pilot(pilotsList[parseInt(String((pilotsList.length - 1) - Math.random() * (pilotsList.length - 1)))])
        }
        ...
        function completude(carrosFinalizados, c = 0) {
          ...
          switch (c) {
            case 1:
              ...
              changePosition(1, `${f}° ${piloto1.getNome} [${carro1.modelo}] <span style='color: deeppink;'>100%</span> - ${tempo.getTempo()}`, f)
              carro1f = true
              f++
              --p
              break;
            case 2:
              ...
              changePosition(2, `${f}° ${piloto2.getNome} [${carro2.modelo}] <span style='color: deeppink;'>100%</span> - ${tempo.getTempo()}`, f)
              carro2f = true
              f++
              --p
              break;
            case 3:
              ...
              changePosition(3, `${f}° ${piloto3.getNome} [${carro3.modelo}] <span style='color: deeppink;'>100%</span> - ${tempo.getTempo()}`, f)
              carro3f = true
              f++
              --p
              break;
            case 4:
              ...
              changePosition(4, `${f}° ${piloto4.getNome} [${carro4.modelo}] <span style='color: deeppink;'>100%</span> - ${tempo.getTempo()}`, f)
              carro4f = true
              f++
              --p;
          }
        ...
        let minhaPosicao = function (distancia = 0, eu = 1) {
          ...
          switch (eu) {
            case 1:
              ...
              changePosition(1, `${minhaPosicaoAgr}° ${piloto1.getNome} [${carro1.modelo}] <span>${(carro1.getDistanciaPecorrida * 100 / limite).toFixed(0)}%</span> - ${(carro1.getVelocidadeAtual).toFixed(0)} km/h`, minhaPosicaoAgr)
              break;
            case 2:
              if (!carro1f) {
              ...
              changePosition(2, `${minhaPosicaoAgr}° ${piloto2.getNome} [${carro2.modelo}] <span>${(carro2.getDistanciaPecorrida * 100 / limite).toFixed(0)}%</span> - ${(carro2.getVelocidadeAtual).toFixed(0)} km/h`, minhaPosicaoAgr)
              break;
            case 3:
              if (!carro1f) {
                if (distancia > carro1.getDistanciaPecorrida) {
              ...
              changePosition(3, `${minhaPosicaoAgr}° ${piloto3.getNome} [${carro3.modelo}] <span>${(carro3.getDistanciaPecorrida * 100 / limite).toFixed(0)}%</span> - ${(carro3.getVelocidadeAtual).toFixed(0)} km/h`, minhaPosicaoAgr)
              break;
            case 4:
              if (!carro1f) {
                if (distancia > carro1.getDistanciaPecorrida) {
              ...
              changePosition(4, `${minhaPosicaoAgr}° ${piloto4.getNome} [${carro4.modelo}] <span>${(carro4.getDistanciaPecorrida * 100 / limite).toFixed(0)}%</span> - ${(carro4.getVelocidadeAtual).toFixed(0)} km/h`, minhaPosicaoAgr)
            }
        }
        ...
        function informacoes() {
          ...
          modelos.forEach((carro, index) => {
            ...
            document.getElementById(`modelo${index + 1}`).innerHTML = `${carro.modelo} <span><i class="fas fa-eye"></i></span>`

            //Mostrando categorias
            document.getElementById(`categoria${index + 1}`).innerHTML = `[${carro.categoria}]`
            document.getElementById(`categoria${index + 1}`).style.background = corCategorias[carro.categoria.toLowerCase()]
            ...
            // info rápida
            document.querySelector(`[info-rapida-velocidade${index + 1}]`).innerHTML = `Vel. max.: ${carro.velocidade} km/h`
            document.querySelector(`[info-rapida-aceleracao${index + 1}]`).innerHTML = `Aceleração: ${carro.delta}`
            document.querySelector(`[info-rapida-resistencia${index + 1}]`).innerHTML = `Resistência: ${carro.qualidade}`
            document.querySelector(`[info-rapida-turbo${index + 1}]`).innerHTML = `Turbo: ${carro.getNitro}`
            document.querySelector(`[info-rapida-forca${index + 1}]`).innerHTML = `Força: ${carro.forca}`
          })
        ...
        modelos.forEach((carro, index) => {
          ...
          runs.push(() => {
            ...
            // if (!carro.getBtnNitro) carro.turbo()
            // else carro.start()
            ...
            document.querySelector(`#ponteiro${index + 1} img`).style.transform = `translate(0px, 0px) rotate(${carro.getVelocidadeAtual * 180 / (carro.velocidade * (carro.getMarcha * 20) / 100)}deg)`
            document.getElementById(`velocidade${index + 1}`).innerHTML = `${(carro.getVelocidadeAtual).toFixed(0)} km/h`
            document.getElementById(`quebra${index + 1}`).innerHTML = `${(100 * carro.getResistencia / carro.qualidade).toFixed(0)}%`
            ...
            } else if (carro.getBtnNitro === true && carro.getSoltandoNitro && carro.getCilindro === true) {
              document.querySelector(`[nitro${index + 1}]`).style.height = `${0 + 100 * (carro.getTanque - carro.getNitro) / (carro.getNitro / 3)}%`
              document.querySelector(`[nitro${index + 1}]`).style.background = `linear-gradient(to bottom, #909090, rgb(255, ${carro.getAproveitamento}, ${carro.getAproveitamento}))`
        ...
        setTimeout(() => {
          ...
          for (let f = 0; f < 4; f++) {
            freios.push(setInterval(() =>{
              modelos[f].frear()
              pararDeFreiar()
            }, nomesPilotos[f].getHabilidade * 1500))
          }
        ...
        function Reset() {
          ...
          setTimeout(() => {
            document.getElementById('musica').innerHTML = `<audio autoplay='true' loop='true'><source src='som/${musics[Math.floor(Math.random() * 2)]}.mp3' type='audio/ogg'></audio>`
